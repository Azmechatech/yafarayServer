...
import net.mk.Shapes.Algos.ModelHelper;
import net.mk.Shapes.Algos.NumericalMasterGeometry;
import org.json.JSONObject;
import pinion.Gear2DGraphicsHelper;

/**
 *
 * @author Manoj
 */
public class ParquetWriteHandler  extends CustomHandler {

    String yafarayPath="C:\\DevTools\\yafaray_v3\\bin\\yafaray-xml";

    public ParquetWriteHandler(String yafarayPath){
    ...
    public void handle(HttpExchange he) throws IOException {
        ...
        try {
            ...
            NumericalMasterGeometry nmg=ModelHelper.generateEngineeringData(model, modelName);
            List<Pair<String,Pair<String,Obj>>> parquetData=new LinkedList<>();
            nmg.getComponentTree().entrySet().stream().forEach(kv->{
                Pair<String,Obj> pairToStore=new Pair<>("COMPONENT",kv.getValue().getObject());
                parquetData.add(new Pair(kv.getKey(), pairToStore));
            });
            //Store information in parquet.
            File storeDir=new File("store/"+Gear2DGraphicsHelper.md5(modelName));storeDir.mkdirs();
            ObjParquetHelper.write(parquetData,storeDir.getAbsolutePath(),modelName);
